...
import { HeadConfig, resolveSiteDataByRoute } from '../shared';

import { SiteConfig } from '../config';
import { slash } from '../utils/slash';

// render md page file to html
export async function renderPage(
    ...
    // prioritize project root over rvpress' own dep
    const rendererPath = require.resolve('react-dom/server');
    ...
    // TODO use title for judge?
    const title: string =
        pageData.title && pageData.title !== 'Home' ? `${pageData.title} | ${siteData.title}` : siteData.title;
...
<html lang="${siteData.lang}">
    ...
    <body>
        ...
        ${config.mpa ? '' : `<script>__RVP_HASH_MAP__ = JSON.parse(${hashMapString})</script>`}
        ${appChunk ? `<script type="module" async src="${siteData.base}${appChunk.fileName}"></script>` : ``}
        ${inlinedScript}
    </body>
</html>`.trim();
...
function resolvePageImports(config: SiteConfig, page: string, result: RollupOutput, appChunk: OutputChunk) {
    // find the page's js chunk and inject script tags for its imports so that
    // they start fetching as early as possible
    const srcPath = normalizePath(fs.realpathSync(path.resolve(config.srcDir, page)));
    const pageChunk = result.output.find(
        chunk => chunk.type === 'chunk' && chunk.facadeModuleId === srcPath
    ) as OutputChunk;
    return [...appChunk.imports, ...appChunk.dynamicImports, ...pageChunk.imports, ...pageChunk.dynamicImports];
}
