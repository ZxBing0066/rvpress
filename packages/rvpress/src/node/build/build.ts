...
// build docs to static
export async function build(root: string, buildOptions: BuildOptions & { mpa?: string } = {}) {
    ...
    try {
        const { clientResult, serverResult, pageToHashMap } = await bundle(siteConfig, buildOptions);
        ...
        try {
            const appChunk =
                clientResult &&
                (clientResult.output.find(chunk => chunk.type === 'chunk' && chunk.isEntry) as OutputChunk);
