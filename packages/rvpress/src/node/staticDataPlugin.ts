...
const idToLoaderModulesMap: Record<string, CachedLoaderModule | undefined> = Object.create(null);
...
// loading.
let idToPendingPromiseMap: Record<string, Promise<string> | undefined> = Object.create(null);
let isBuild = false;
...
export const staticDataPlugin: Plugin = {
    ...
    async load(id) {
        if (loaderMatch.test(id)) {
            ...
            if (existing) {
                ...
                const loaderModule = (await loadConfigFromFile({} as any, id))?.config as LoaderModule;
    ...
    handleHotUpdate(ctx) {
        for (const id in idToLoaderModulesMap) {
            ...
            if (isLoaderFile || (pattern && isMatch(relative(base, ctx.file), pattern))) {
