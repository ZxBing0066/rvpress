...
export function wrapCls(cls: Record<string, string>, wrapCls: string, className?: string): string {
    return `${wrapCls} ${cls[wrapCls]}` + (className ? ` ${className}` : '');
}
